...
// ----------------------------------------------------------------------------
  ...
  vec3 CalculateDirectional(vec3 LightDirection, vec3 LightColor)
{
 ...
 vec3 CalculateDirectionalShadow(vec3 LightDirection, vec3 LightColor, sampler2D shadowMap, vec4 fragLightPos)
{
 ...
 vec3 CalculatePoint(vec3 LightPosition, vec3 LightColor)
{
  ...
  vec3 CalculateSpot(vec3 LightDirection,vec3 LightPosition, vec3 LightColor, float OuterAngle, float InnerAngle)
{
 ...
 vec3 CalculateSpotShadow(vec3 LightDirection,vec3 LightPosition, vec3 LightColor, float OuterAngle, float InnerAngle,sampler2D shadowMap, vec4 fragLightPos)
{
...
void main()
{
    ...
    vec3 color = vec3(0.0,0.0,0.0);
     ...
     {
       color += CalculateDirectional(u_LightDirDirection[i],u_LightDirColor[i]);
     }

     for(int i = 0; i < u_NumDirectionalsShadow; i++)
     {
       color += CalculateDirectionalShadow(u_LightDirDirectionShadows[i],u_LightDirColorShadows[i],u_shadowMapDir[i],FragLightPosDir[i]);
     }

     for(int i = 0; i < u_NumSpots; i++)
    {
        color += CalculateSpot(u_LightSpotDirection[i],u_LightSpotPosition[i],u_LightSpotColor[i],u_LightSpotOuterAngle[i],u_LightSpotInnerAngle[i]);
    }

    for(int i = 0; i < u_NumSpotsShadow; i++)
    {
        color += CalculateSpotShadow(u_LightSpotDirectionShadows[i],u_LightSpotPositionShadows[i],u_LightSpotColorShadows[i],u_LightSpotOuterAngleShadows[i],u_LightSpotInnerAngleShadows[i],u_shadowMapSpot[i],FragLightPosSpot[i]);
    }

    for(int i = 0; i < u_NumPoints; i++)
    {
        color += CalculatePoint(u_LightPointPosition[i],u_LightPointColor[i]);
    }
