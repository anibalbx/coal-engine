...
void coalengine::DrawCubeMap::run()
{
  ...
  if (!RL.sky_box_.uploaded_to_gpu_)
  {
    ...
    uint32 size = RL.sky_box_.vertices_.size() * sizeof(float) + RL.sky_box_.normals_.size() * sizeof(float) + RL.sky_box_.uv_.size() * sizeof(float);
  ...
  glUniform1i(glGetUniformLocation(RL.sky_box_.program_id_, "u_activebloom"),
